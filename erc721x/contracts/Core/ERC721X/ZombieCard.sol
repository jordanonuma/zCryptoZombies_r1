...
Contract ZombieCard is ERC721XToken {
  ...
  function batchMintTokens(uint[] _tokenIds, uint[] _tokenSupplies) external onlyOwner {
    ...
    } //end for(each token to be batch minted)
  ...
  function convertToFT(uint _tokenId) public {
    ...
    _updateTokenBalance(msg.sender, _tokenId, 0, ObjectLib.Operations.REPLACE); //updates NFT balance to 0
    _updateTokenBalance(msg.sender, nftTokenIdtoMouldId[_tokenId], 1, ObjectLib.Operations.ADD); //increments 1 to FT mould balance
    emit TransferWithQuantity(address(this), msg.sender, nftTokenIdToMouldId[_tokenId], 1); //emiting this event lets the user know conversion was successful
